...
CREATE POLICY "Users can read own issues" ON public.issues FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Mechanics can read open issues" ON public.issues FOR SELECT USING (has_role(auth.uid(), 'mechanic'));
CREATE POLICY "Users can insert issues" ON public.issues FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update own issues" ON public.issues FOR UPDATE USING (user_id = auth.uid());
...
CREATE POLICY "Mechanics can read consented profiles" ON public.profiles FOR SELECT USING (
  has_role(auth.uid(), 'mechanic') AND EXISTS (
    SELECT 1 FROM phone_share_consents psc
    WHERE psc.user_id = profiles.user_id AND psc.mechanic_id = auth.uid() AND psc.granted = true
  )
);
...
CREATE POLICY "Anyone authed can read mechanic profiles" ON public.mechanic_profiles FOR SELECT USING (true);
CREATE POLICY "Mechanics can insert own profile" ON public.mechanic_profiles FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Mechanics can update own profile" ON public.mechanic_profiles FOR UPDATE USING (user_id = auth.uid());
...
CREATE POLICY "Read responses" ON public.mechanic_responses FOR SELECT USING (
...
CREATE POLICY "Mechanics can insert responses" ON public.mechanic_responses FOR INSERT WITH CHECK (mechanic_id = auth.uid());
CREATE POLICY "Mechanics can update own responses" ON public.mechanic_responses FOR UPDATE USING (mechanic_id = auth.uid());
...
CREATE POLICY "Participants can read messages" ON public.messages FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM issues i WHERE i.id = messages.issue_id AND (
      i.user_id = auth.uid() OR EXISTS (
        SELECT 1 FROM mechanic_responses mr WHERE mr.issue_id = i.id AND mr.mechanic_id = auth.uid()
      )
    )
  )
);
...
CREATE POLICY "Users can read own vehicles" ON public.vehicles FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Mechanics can read issue vehicles" ON public.vehicles FOR SELECT USING (has_role(auth.uid(), 'mechanic'));
CREATE POLICY "Users can insert vehicles" ON public.vehicles FOR INSERT WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can manage own consents" ON public.phone_share_consents FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Mechanics can read granted consents" ON public.phone_share_consents FOR SELECT USING (mechanic_id = auth.uid() AND granted = true);
